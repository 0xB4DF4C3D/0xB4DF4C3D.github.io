...
DSL이 기존의 전화 회사 인프라를 사용한다면, **cable Internet access** 는 기존의 케이블 텔레비전 회사 인프라를 사용한다. 주택은 케이블 텔레비전을 제공하는 회사로부터 케이블 인터넷 접근을 제공 받는다. 광섬유는 cable head end와 neighborhood-level junction들을 연결한다. 그다음 거기서부터 기존의 동축 케이블이 각각의 집과 아파트들에 도달하기 위해 사용된다. 각 neighborhood-junction들은 일반적으로 500에서 5000개의 가구들을 지원한다. 이 시스템에서 광섬유와 동축케이블이 둘 다 사용되기 때문에, 종종 hybirid fiber coax(HFC)라고도 나타내진다. 

케이블 인터넷 접근은 케이블 모뎀이라는 특별한 모뎀을 필요로 한다. DSL 모뎀에서처럼, 케이블 모뎀은 일반적으로 외부 장치고 이더넷 포트를 거쳐 가정 PC에 연결된다. (6장에서 이더넷에 대해 아주 자세하게 다룰것이다.) cable head end에서, cable modem termination system(CMTS)은 DSL 네트워크의 DSLAM과 유사한 기능을 수행한다.&mdash; 다운로드하는 많은 가정의 케이블 모뎀에서 보내진 아날로그 신호를 다시 디지털 형태로 복구한다. 케이블 모뎀은 HFC 네트워크를 다운로드와 업로드 두 채널로 나눈다. DSL처럼 접근은 일반적으로 다운로드 채널이 일반적으로 업로드 채널보다 더 높은 전송 속도를 차지하는 비대칭적이다. DOCSIS 2.0 표준은 다운로드 속도를 최대 42.8Mbps 업로드 속도를 최대 30.7Mbps로 정의했다. DSL 네트워크의 경우처럼, 최대로 나올 수 있는 속도는 낮게 계약된 데이터 속도 또는 매개체 장애로 인해 실현될 수 없을 것이다.

케이블 인터넷 접근의 중요한 특성 중 하나는 전파 매체(broadcast media)를 공유한다는 것이다. 특히, 다운로드 시, head end로부터 전송된 모든 패킷들은 모든 가정에 모든 회선으로 전달되고 업로드 시, 가정에서 전송된 모든 패킷들은 headend로 전달 된다. 이러한 연유로 몇몇 사용자들이 동시에 동영상을 다운 받을 때, 각 사용자가 받는 동영상 파일의 속도는 회선의 다운로드 속도 총합보다 확연히 느릴 것이다. 반면에, 오직 몇몇 사용자들만이 있고 그들이 모두 웹 서핑을 한다면, 사용자들이 정확히 동시에 웹 페이지를 요청하기란 드문 일이므로 최대 케이블 다운로드 속도로 웹 페이지들을 받을 수 있을 것이다. 업로드 채널 역시 공유되기 때문에, 분산 다중 접근 프로토콜은 전송을 조정하고 충돌을 피하기 위해 필요하다.(6장에서 충돌 문제에 대해 좀 자세히 살펴볼 것이다.)

비록 DSL과 케이블 네트워크가 현재 미국의 residential 광대역 접근 중 85퍼센트 이상을 차지하지만, 이보다 더 높은 속도를 제공하는 전도유망한 기술은 **fiber-to-the-home(FTTH)** 이다. 이름이 암시하듯이, FTTH 개념은 간단하다.&mdash; CO에서 가정까지 직접적으로 광섬유 경로를 제공하는 것. 오늘날 많은 국가들&mdash;아랍 에미리트, 남한, 홍콩, 일본, 싱가포르, 타이완, 리투아니아 그리고 스웨덴을 포함하여&mdash;은 이제 가정 보급률이 30%를 넘었다.
...
여기선 간략하게 Verizon의 FIOS 서비스에서 사용되는 PON에 대해 알아본다. 각 가정은 neighborhood splitter까지 전용 광섬유로 연결되는 optical network terminator(ONT)를 가진다. splitter는 여러 가정(보통 100가구 미만)을 전화회사 CO의 optical line terminator까지 연결하는 하나의 공유 광섬유로 통합한다. 빛과 전기 신호 사이에서 변환을 제공하는 OLT는 전화회사 라우터를 통해 인터넷에 연결된다. 가정에선, 사용자는 가정 라우터(일반적으로 무선 라우터)를 ONT까지 연결하고 이 가정 라우터를 통해 인터넷에 접근한다. PON 구조에선, OLT에서 splitter까지 보내진 모든 패킷들은 splitter에서 복제된다. (cable head end와 유사)

FTTH 잠재적으로 초당 기가 비트속도의 인터넷 접근 속도를 제공할 수 있다. 그러나 대부분의 FTTH ISP들은 더 높은 속도에 더 비싼 비용을 받으며 각기 다른 속도를 제공한다. 2011년 미국 FTTH 사용자들의 평균 다운로드 속도는 20Mbps 였다.

다른 두 액세스 네트워크 기술들 또한 가정에 인터넷 접근을 제공하기 위해 사용된다. DSL, 케이블, 그리고 FTTH를 사용할 수 없는 장소(시골 같은)에서는 위성 통신이 1Mbps 이상의 속도로 거주자와 인터넷을 연결 할 수 있다; Starband 그리고 HoughesNet들은 그러한 두 위성 접근 제공자들이다. 기존 전화선을 통해 Dial-up로 접근하는 것은 DSL과 같은 모델에 기반한다.&mdash; 전화선을 거쳐 ISP의 모뎀까지 연결된 가정 모뎀. DSL과 다른 광대역 액세스 네트워크와 비교하여, dial-up 접근은 56kbps로 정말 느리다.